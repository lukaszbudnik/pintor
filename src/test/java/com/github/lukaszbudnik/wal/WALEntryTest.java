...
class WALEntryTest {
  ...
  void testApplicationDataFormats() {
    ...
    String[] deleteParts =
        new String(deleteEntry.getDataAsBytes())
            .split("\\|", -1); // Use -1 to include trailing empty strings
    ...
    WALEntry rollbackEntry =
        new WALEntry(4L, Instant.now(), ByteBuffer.wrap(rollbackData.getBytes()));
  ...
  void testTimestampPrecision() {
    Instant preciseTimestamp =
        Instant.ofEpochSecond(1609459200, 123456789); // 2021-01-01 00:00:00.123456789 UTC
  ...
  void testLargeData() {
    ...
    WALEntry entry =
        new WALEntry(1L, Instant.now(), ByteBuffer.wrap(largeData.toString().getBytes()));
