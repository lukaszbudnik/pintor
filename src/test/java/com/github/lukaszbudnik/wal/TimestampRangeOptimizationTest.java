...
class TimestampRangeOptimizationTest {
    ...
    void testTimestampRangeOptimizationWithMultipleFiles() throws WALException, InterruptedException {
        ...
        for (WALEntry entry : entriesFromMidTime2) {
            assertTrue(entry.getSequenceNumber() >= 14, 
        ...
        for (WALEntry entry : earlyTimeRange) {
            assertTrue(entry.getSequenceNumber() <= 13, 
                "Early entry sequence " + entry.getSequenceNumber() + " should be <= 13");
        }
        
        // Test 4: Read all entries by timestamp
        List<WALEntry> allEntriesByTime = wal.readRange(startTime.minusSeconds(1), endTime.plusSeconds(1));
    ...
    void testTimestampRangeWithFileRotation() throws WALException, InterruptedException {
        ...
        for (WALEntry entry : fromMid) {
            assertTrue(entry.getSequenceNumber() > lastSeq, 
    ...
    void testTimestampRangeAfterRestart() throws Exception {
        ...
        List<WALEntry> afterRestart = wal.readRange(beforeRestart.minusMillis(10), afterEntries.plusMillis(10));
        ...
        Instant midTime = beforeRestart.plusMillis((afterEntries.toEpochMilli() - beforeRestart.toEpochMilli()) / 2);
    ...
    @Test
    void testOptimizedTimestampRangeBuildingWithDerivedMaxValues() throws WALException, InterruptedException {
        ...
        boolean hasEarlyEntries = earlyEntries.stream()
            .anyMatch(entry -> entry.getSequenceNumber() < 15);
        ...
        boolean hasLateEntries = lateEntries.stream()
            .anyMatch(entry -> entry.getSequenceNumber() > 5);
