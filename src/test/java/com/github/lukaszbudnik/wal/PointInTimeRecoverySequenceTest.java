...
/**
 * Integration test demonstrating sequence-based reading capabilities
 * for point-in-time recovery scenarios using sequence numbers.
 */
class PointInTimeRecoverySequenceTest {
    ...
    void testSequenceBasedPointInTimeRecovery() throws WALException {
        ...
        walManager.createEntry("INSERT|txn_1001|users|1|{\"name\":\"Alice\",\"email\":\"alice@example.com\"}".getBytes());
        walManager.createEntry("INSERT|txn_1001|users|2|{\"name\":\"Bob\",\"email\":\"bob@example.com\"}".getBytes());
        ...
        walManager.createEntry("UPDATE|txn_1002|users|1|{\"name\":\"Alice Smith\",\"email\":\"alice.smith@example.com\"}".getBytes());
        ...
        walManager.createEntry("INSERT|txn_1003|users|3|{\"name\":\"Charlie\",\"email\":\"charlie@example.com\"}".getBytes());
    ...
    void testSequenceBasedBatchRecovery() throws WALException {
        ...
        List<ByteBuffer> batchData = Arrays.asList(
            ...
            ByteBuffer.wrap("INSERT|txn_batch|products|1|{\"name\":\"Laptop\",\"price\":999.99}".getBytes()),
            ByteBuffer.wrap("INSERT|txn_batch|products|2|{\"name\":\"Mouse\",\"price\":29.99}".getBytes()),
            ByteBuffer.wrap("INSERT|txn_batch|products|3|{\"name\":\"Keyboard\",\"price\":79.99}".getBytes()),
    ...
    void testSequenceBasedCheckpointRecovery() throws WALException {
        ...
        for (int i = 0; i < 10; i++) {
            assertTrue(new String(afterCheckpoint.get(i).getDataAsBytes()).contains("OPERATION|" + (10 + i)));
