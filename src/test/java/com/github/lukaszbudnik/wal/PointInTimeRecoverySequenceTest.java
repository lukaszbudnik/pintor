...
class PointInTimeRecoverySequenceTest {
  ...
  void testSequenceBasedPointInTimeRecovery() throws WALException {
    ...
    long phase1EndSeq = wal.getCurrentSequenceNumber(); // Should be 3
    ...
    long phase2EndSeq = wal.getCurrentSequenceNumber(); // Should be 7
    ...
    long phase3EndSeq = wal.getCurrentSequenceNumber(); // Should be 10
  ...
  void testSequenceBasedCheckpointRecovery() throws WALException {
    ...
    for (int i = 0; i < 10; i++) {
      wal.createAndAppend(ByteBuffer.wrap(("OPERATION|" + i + "|data" + i).getBytes()));
    }

    long checkpointSeq = wal.getCurrentSequenceNumber(); // 9
  ...
  void testSequenceBasedRollbackRecovery() throws WALException {
    ...
    wal.createAndAppend(ByteBuffer.wrap("BEGIN|txn_good|".getBytes()));
    wal.createAndAppend(ByteBuffer.wrap("INSERT|txn_good|users|1|Alice".getBytes()));
    wal.createAndAppend(ByteBuffer.wrap("COMMIT|txn_good|".getBytes()));

    long goodTransactionEnd = wal.getCurrentSequenceNumber(); // 2

    // Failed transaction (that needs to be rolled back)
    wal.createAndAppend(ByteBuffer.wrap("BEGIN|txn_bad|".getBytes()));
    wal.createAndAppend(ByteBuffer.wrap("INSERT|txn_bad|users|2|BadData".getBytes()));
    wal.createAndAppend(ByteBuffer.wrap("UPDATE|txn_bad|users|1|CorruptedAlice".getBytes()));
