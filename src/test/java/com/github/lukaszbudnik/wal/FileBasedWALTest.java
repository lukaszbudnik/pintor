...
class FileBasedWALTest {
    ...
    void testAppendOperations() throws WALException {
        ...
        // Test batch append
        List<ByteBuffer> batchData = Arrays.asList(
            ByteBuffer.wrap("DELETE|txn_1003|data3".getBytes()),
            null // Test null data
        );
    ...
    void testDataFormats() throws WALException {
        ...
        for (int i = 0; i < testData.length; i++) {
            ...
            for (int j = 0; j < originalParts.length; j++) {
                assertEquals(originalParts[j], parts[j], 
                    "Entry " + i + " part " + j + " mismatch");
            }
            
            // Verify operation type is valid
            String operation = parts[0];
            assertTrue(operation.matches("INSERT|UPDATE|DELETE|BEGIN|COMMIT|ROLLBACK|CHECKPOINT|CUSTOM"),
                "Invalid operation: " + operation);
            
            // Verify transaction format when present
            if (parts.length > 1 && !parts[1].isEmpty()) {
                assertTrue(parts[1].startsWith("txn_") || parts[1].isEmpty(),
    ...
    void testDataIntegrity() throws WALException {
        ...
        String complexData = "UPDATE|txn_9999|users|42|Complex data with special chars: àáâãäåæçèéêë ñ 中文 🚀";
        ...
        assertTrue(Math.abs(originalEntry.getTimestamp().getEpochSecond() - readEntry.getTimestamp().getEpochSecond()) <= 1);
    ...
    void testTruncateOperations() throws WALException {
        ...
        WALEntry newEntry = wal.createAndAppend(ByteBuffer.wrap("INSERT|txn_new|after_truncate".getBytes()));
    ...
    void testLargeDataHandling() throws WALException {
        ...
        WALEntry structuredEntry = wal.createAndAppend(ByteBuffer.wrap(structuredLargeData.toString().getBytes()));
    ...
    void testCreateAndAppendWithEmptyBatch() throws WALException {
        // Test empty batch
        List<WALEntry> entries = wal.createAndAppendBatch(Arrays.asList());
    ...
    void testMixedOperations() throws WALException {
        ...
        // Use batch API
        List<WALEntry> batchEntries = wal.createAndAppendBatch(Arrays.asList(
            ByteBuffer.wrap("batch_1".getBytes()),
            ByteBuffer.wrap("batch_2".getBytes())
        ));
    ...
    @Test
    void testTimestampBasedReadingAcrossFiles() throws WALException, InterruptedException, Exception {
