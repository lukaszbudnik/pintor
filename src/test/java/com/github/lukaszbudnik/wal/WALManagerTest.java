...
class WALManagerTest {
  ...
  @Mock private WriteAheadLog mockWal;
  ...
  void testCreateEntryWithByteBuffer() throws WALException {
    ...
    WALEntry expectedEntry =
        new WALEntry(5L, Instant.now(), ByteBuffer.wrap("test data".getBytes()));
  ...
  void testCreateEntryWithNullData() throws WALException {
    // Given
    WALEntry expectedEntry = new WALEntry(0L, Instant.now(), null);
  ...
  void testCreateEntryCallsAppend() throws WALException {
    ...
    verify(mockWal)
        .createAndAppend(
            argThat(buffer -> buffer != null && Arrays.equals(getBufferBytes(buffer), data)));
  }
  ...
  void testCreateEntryThrowsExceptionOnAppendFailure() throws WALException {
    ...
    // When & Then
    WALException actualException =
        assertThrows(WALException.class, () -> walManager.createEntry("data".getBytes()));
  ...
  void testTruncateThrowsException() throws WALException {
    ...
    // When & Then
    WALException actualException =
        assertThrows(WALException.class, () -> walManager.truncate(100L));
    assertEquals(expectedException, actualException);
    verify(mockWal).truncate(100L);
  }
  ...
  void testCreateEntryWithApplicationData() throws WALException {
    ...
    WALEntry expectedEntry =
        new WALEntry(7L, Instant.now(), ByteBuffer.wrap(applicationData.getBytes()));
    when(mockWal.createAndAppend(any(ByteBuffer.class))).thenReturn(expectedEntry);
  ...
  void testCreateEntryBatchFromBytes() throws WALException {
    ...
    List<byte[]> dataList =
        Arrays.asList(
            "data1".getBytes(), "data2".getBytes(), null // Test null data
            );
    ...
    List<WALEntry> expectedEntries =
        Arrays.asList(
            ...
            new WALEntry(12L, Instant.now(), null));
